...
It is common to want to create a task and schedule it right away, so the macro [`@async`](/base/parallel#Base.@async) is provided for that purpose –- `@async x` is equivalent to `schedule(@task x)`.
